...
// These are used throughout the compiler code for consistency with the C# original.
//
//goland:noinspection GoUnusedGlobalVariable
var (
    CBasicTypeSignedInt            CType = SignedInt
    CBasicTypeBool                 CType = Bool
    CBasicTypeFloat                CType = Float
    CBasicTypeDouble               CType = Double
    CBasicTypeUnsignedLongInt      CType = UnsignedLongInt
    CBasicTypeSignedLongInt        CType = SignedLongInt
    CBasicTypeSignedLongLongInt    CType = SignedLongLongInt
    CBasicTypeUnsignedLongLongInt  CType = UnsignedLongLongInt
    CBasicTypeUnsignedInt          CType = UnsignedInt
    CBasicTypeUnsignedChar         CType = UnsignedChar
    CBasicTypeSignedChar           CType = SignedChar
    CBasicTypeUnsignedShortInt     CType = UnsignedShortInt
    CBasicTypeSignedShortInt       CType = SignedShortInt
    CPointerTypePointerToConstChar CType = PointerToConstChar
)
...
func (ec *EmitContext) Emit(op OpCode, x Value) {
    ec.self.EmitInstruction(Instruction{Op: int(op), X: x})
}

func (ec *EmitContext) EmitBranch(op OpCode, label *Label) {
    ec.self.EmitInstruction(Instruction{Op: int(op), Label: label})
}
...
func (ec *EmitContext) MakeCFunctionType(returnType CType, decl Declarator, block *Block) CType {
    ...
    if id, ok := fdecl.InnerDeclarator.(*IdentifierDeclarator); ok && len(id.Context) > 0 {
        ...
        declaringType = ec.ResolveTypeNameString(declaringTypeName)
    }
...
func (ec *EmitContext) MakeCTypeFromSpecs(specs *DeclarationSpecifiers, init Initializer, block *Block) CType {
        ...
        if structTs.Body != nil {
            ...
            if structTs.BaseSpecifiers != nil {
                for _, baseSpec := range structTs.BaseSpecifiers {
                    ...
                    baseType := ec.ResolveTypeNameString(baseSpec.Name)
    ...
    if typenameTs != nil {
        return ec.ResolveTypeNameString(typenameTs.Name)
    }
